...
class MainPage : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val meal_carbonara = Meal("1","Карбонара", null, 30, "лёгкая",
            1031, 104,
            listOf(Ingredient("1", 0.125), Ingredient("2", 0.05),
           ...
           ("5", 0.05), Ingredient("6", 0.05), Ingredient("7", 0.025),
            ...
            703, 37, 30, 71, listOf("sports"), listOf("month", "season"), listOf(
                InstructionStep(listOf(Ingredient("1")), "Вскипятите воду в большой кастрюле и сварите пасту до состояния аль денте. Обычно для этого нужно варить ее на минуту меньше, чем указано на пачке."),
                InstructionStep(listOf(Ingredient("2"), Ingredient("4"), Ingredient("3"), Ingredient("5")), "Пока паста варится, растопите на сковороде масло и обжарьте на нем мелко нарезанные лук, чеснок и бекон. До мягкости и до отчетливого чесночного и жаренобеконного запаха."),
                InstructionStep(listOf(Ingredient("8"), Ingredient("6"), Ingredient("7"), Ingredient("9"), Ingredient("10")), "Снимите сковороду с огня и в глубокой миске взбейте четыре яичных желтка со сливками и тертым пармезаном. Посолите и поперчите смесь, еще раз взбейте."),
                InstructionStep(listOf(Ingredient("10")), "В готовые спагетти вывалите обжаренные с луком и чесноком кусочки бекона. Влейте смесь сливок, желтков и пармезана, перемешайте. И сразу подавайте, посыпав свеженатертым сыром и черным молотым перцем.")
            ))
...
//                    spanCount
//                }
        val monthMealsItems = meals.filter { it.tags?.contains("month") ?: false }.sortedByDescending { it.likes }.take(6)
        ...
        val interestingMealItems = meals.filter { it.tags?.contains("interesting")?:false }.take(6)
...
//        Database connection for downloading Meal list
//        db.collection("Meal")
//            .get()
